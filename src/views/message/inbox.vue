<template>
  <div class="app-small">
    <div class="listTotal">收件箱（共{{total}}条）</div>
    <table-operate >
      <el-button type="danger" size="small" v-if="hasAuth('inboxDelete')" @click="batchRemove">删除</el-button>
      <el-button type="primary" size="small" v-if="hasAuth('inboxRead')" @click="read">标记已读</el-button>
    </table-operate>
    <table-search>
    ...
    <div class="app-container">
      <el-table :data="list" :height="tableHeight" v-loading.body="listLoading" element-loading-text="拼命加载中" @selection-change="handleSelectionChange"  border fit stripe style="with:100%">
        ...
        <el-table-column prop="sendName" label="发送者" min-width="100">
          <template slot-scope="scope">
            {{scope.row.sendName}}<{{scope.row.sendNum}}>
          </template>
        </el-table-column>
        <!--<el-table-column prop="receivedName" label="接收人" />-->
        ...
        <el-table-column prop="createTime" label="发送时间"  :formatter="dateFormat" width="160"/>
        <el-table-column prop="address" label="发送地址" min-width="180"/>
        ...
        <el-table-column label="操作" align="center" width="200">
          <template slot-scope="scope">
            <el-button size="medium" style="color:#4687BE" type="text" v-if="hasAuth('inboxRelay') && hasAuth('message:outbox')" @click="replyTo(scope.row.content)">转发</el-button>
            <el-button size="medium" style="color:#4687BE" type="text" v-if="hasAuth('inboxAnswer') && hasAuth('message:outbox')" @click="reply(scope.row.sendName,scope.row.sendNum)">回复</el-button>
            <el-button size="medium" style="color:#F78989" type="text" v-if="hasAuth('inboxDelete')" @click="remove(scope.row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      reply(data,sendNum) {
       this.$router.push({path:'/message/outbox', query: {sendNum: sendNum,returnUrl:'/message/inbox'}})
      },
      replyTo(data) {
       this.$router.push({path:'/message/outbox', query: {content: data,returnUrl:'/message/inbox'}})
      },
      remove(id) {
